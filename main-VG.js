...
//console.log(getCoordinates(tiles[59]));
...
function removeShip(ship) {
  ship.forEach(function (coord) {
    const tile = tiles.find(function (tile) {
...
function displayMarkersOnGrid(markers) {
  markers.forEach((marker) => {
    const tile = tiles.find((tile) => {
      const { row, col } = getCoordinates(tile);
      return row === marker.row && col === marker.col;
    });
    if (tile) {
      addMark(tile, marker.marker);
    }
  });
}
...
function isValidShip(ship) {
  ...
  const rowCoordinates = ship.map((coord) => coord.row);
  const colCoordinates = ship.map((coord) => coord.col);

  const sameRow = rowCoordinates.every((val) => val === rowCoordinates[0]);
  const sameCol = colCoordinates.every((val) => val === colCoordinates[0]);
...
function hasLost(player) {
  return player.ships.every((shipCoord) =>
    player.hits.some(
      (hitCoord) =>
        hitCoord.row === shipCoord.row && hitCoord.col === shipCoord.col
    )
  );
}
